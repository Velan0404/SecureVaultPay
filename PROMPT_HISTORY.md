...
Phase 8 – Analytics & Insights Module. Requested reading all six documentation files and reviewing every completed module first, with no changes to Authentication, Wallet, Merchant Payment, QR Payment, Personal Payment, or Scheduled Payment business logic unless required for integration. Goal: a complete Analytics & Insights module using only existing transaction data, no duplicated data. Dashboard Analytics: Total Balance, Total Income, Total Expenses, Total Transfers, Total Merchant Payments, Total User Payments, Total Scheduled Payments, Monthly Spending. Purpose Wallet Analytics: per wallet, Current Balance, Total Deposited, Total Spent, Remaining Budget, Spending Percentage, Number of Transactions. Charts (`fl_chart`, matching the design system): Monthly Spending Line, Purpose Wallet Pie, Weekly Expense Bar, Income vs Expense. Insights: real, rule-generated observations (budget %, trend change, dormant wallet, upcoming payment), never fabricated. Filters: Today / Last 7 Days / Last 30 Days / Last 90 Days / This Year / Custom Range, with automatic refresh. Reports: Daily/Weekly/Monthly summaries of income/expenses/transfers/merchant/personal/scheduled payments. Backend: complete module (repository/service/controller/routes/validation), reusing existing transaction tables, no duplicated calculations. Flutter: Analytics Dashboard / Purpose Wallet Analytics / Monthly Report / Insights screens. Performance: optimized queries, no N+1, efficient aggregation. Verify `flutter analyze` clean, backend boots, charts render, filters work, reports calculate correctly, existing modules unaffected, Dashboard analytics update automatically. Update all three documentation files; stop after this phase.

Result:

Entered plan mode given the genuine architectural decisions involved (how to categorize income/expense from existing enum values with no explicit flag, how "Total Scheduled Payments" should differ from the Merchant/User Payment totals it overlaps with, chart granularity) — explored the Wallet/Merchant/Personal/Scheduled repositories and services plus the Flutter dashboard/provider patterns via two parallel Explore agents first, then confirmed three specific interpretive calls with you via `AskUserQuestion` (all three "Recommended" options accepted) before writing the plan: Income = `DEMO_LOAD` + `PERSONAL_PAYMENT_RECEIVED`, Expense = `PURPOSE_PAYMENT` + `PERSONAL_PAYMENT_SENT`, transfers counted separately; Total Scheduled Payments = sum of `ScheduledPaymentExecution` rows with `status=SUCCESS` (a different lens on money already counted elsewhere, not a double-count); the Weekly Expense chart is one bar per day for the last 7 days.

The core design realization: every existing payment/transfer module already writes a row into `WalletTransaction`, tagged by `type` and (when relevant) `purposeWalletId` — so `WalletTransaction` grouped by `type`/`purposeWalletId` turns out to answer nearly the entire spec's data needs on its own, without touching `MerchantPayment`/`PersonalPayment` directly. This is the first `groupBy` used anywhere in this codebase (`analytics.repository.js`'s `groupTransactionsByWalletAndType(userId, {start, end})`) — one query, covering every wallet and every type at once, from which `analytics.service.js` derives, in memory: the Dashboard's income/expense/transfer/merchant/user totals (summed across wallets), and each wallet's deposited/spent/transaction-count (grouped by wallet) — the alternative would have been N per-wallet queries, which the task explicitly asked to avoid. The Purpose Wallet Pie Chart reuses this exact same per-wallet spend figure a second time with no additional query, and the Income vs Expense chart reuses the Dashboard's own totals the same way — three of the four chart datasets and the two headline screens are all one shared computation, viewed differently. Total Scheduled Payments needed one more small `scheduledPaymentExecution.aggregate()` call, since that table has no `type`/`purposeWalletId` shape compatible with the shared query. `walletService.getDashboard(userId)` (unmodified) supplies `mainWalletBalance`/`totalAllocated`/the wallet list for Total Balance and Purpose Wallet Analytics identity — no wallet-fetching logic was duplicated. New backend module: `analytics.repository/service/controller/routes.js` plus a shared `utils/dateRange.js` (`resolveRange()` for the six filter presets, same plain-`Date`-mutation idiom `scheduledPayment.service.js`'s dashboard summary already established — no new date library). Five read-only `GET` endpoints (`/dashboard`, `/wallets`, `/charts`, `/insights`, `/reports`) behind a new `analyticsLimiter`, mounted at `/api/analytics`. `getReport()` (Reports) and `getDashboardAnalytics()` (Dashboard) share one `computeTotals()` helper — one aggregation function, two callers, so "do not duplicate calculations" holds at the code level, not just the data level. The Insights engine reuses `computeWalletAnalytics()`'s already-computed `spendingPercentage`/`totalSpent` for budget and trend insights, `scheduledPaymentService.getDashboardSummary()` verbatim (unmodified) for upcoming-payment insights, and only two genuinely new small queries: a prior-equal-length-period comparison (for trend detection) and an all-time `_max(createdAt)` per wallet (for dormancy, deliberately never scoped to the selected filter window, since "hasn't been used in 30 days" is about real-world recency regardless of whatever range happens to be selected on screen). No file under `wallet.*`, `merchant.*`, `personalPayment.*`, or `scheduledPayment.*` was modified — Analytics only calls their already-public functions.

Flutter: new `models/analytics_dashboard_model.dart`/`purpose_wallet_analytics_model.dart`/`analytics_charts_model.dart`/`insight_model.dart`/`analytics_report_model.dart`/`analytics_range.dart` (a shared enum mirroring the backend's six presets), `services/analytics_service.dart`, `repositories/analytics_repository.dart`, `providers/analytics_provider.dart` — the same Service→Repository→Provider layering every module uses, with one `AnalyticsNotifier.loadAll()` fetching all four datasets together on every range change, which is what makes "Analytics should refresh automatically" true (every Analytics screen reads this same shared state rather than fetching independently). Loaded the `dataviz` skill before writing any chart code, then ran its palette validator (`scripts/validate_palette.js`) against this app's actual shipped colors rather than inventing a new palette: confirmed the existing `success`/`danger` pair (already used everywhere else in the app for positive/spending semantics) passes CVD-safety comfortably for the Income vs Expense chart, and — a genuinely useful finding — that the app's six `category*` colors include one adjacent pair (`categoryBlue`/`categoryPurple`) too close together for colorblind users; since each Purpose Wallet already carries its own persistent, user-chosen color everywhere else in the app, the Purpose Wallet Pie Chart reuses that same per-entity color rather than assigning from the six-color set, and always ships with a legend (name + amount) as secondary encoding regardless of hue similarity. Four new chart widgets (`monthly_spending_line_chart.dart`, `weekly_expense_bar_chart.dart`, `purpose_wallet_pie_chart.dart`, `income_vs_expense_chart.dart`) — `fl_chart`'s first real use in this codebase (previously an unused dependency) — thin 2px lines/bars, rounded data-ends, recessive gridlines, touch tooltips, direct value labels on the two-series Income/Expense chart. New `screens/analytics/`: `AnalyticsDashboardScreen` (replaces the deleted stub `analytics_screen.dart` at `/analytics` — a range-filter dropdown, 8 stat tiles, the 4 charts, an Insights preview, links to the other two screens), `PurposeWalletAnalyticsScreen`, `MonthlyReportScreen` (period + date picker), `InsightsScreen` (reusing a new shared `InsightTile` widget for both the Dashboard's preview and this full list). Three new static routes added to `app_router.dart` (`/analytics/wallets`, `/analytics/report`, `/analytics/insights`), no ordering risk since none sit under a dynamic segment.

Testing: a live E2E script against Neon seeded a known transaction history (demo load, two Main-to-Purpose transfers, a merchant payment, a personal payment, and a manually-inserted scheduled execution across two Purpose Wallets, one with a spending limit and one without) and confirmed every figure by hand-computed arithmetic: Dashboard totals (income/expenses/transfers/merchant/user/scheduled/balance) all matched exactly; per-wallet deposited/spent/remaining-budget/spending-percentage were correct, including the `spendingLimit == null` case correctly returning `null` rather than dividing by zero; the pie chart's per-wallet value matched the wallet-analytics total exactly (proving the shared computation, not a second independent one); the weekly bar chart produced exactly 7 buckets with the correct same-day total; no budget insight fired below the 50% threshold and no dormant-wallet insight fired for wallets used the same day; the daily report matched the same-day totals. `flutter analyze` found 2 minor `use_null_aware_elements` info-lints on the first pass (in the new shared query-string builder), fixed immediately with the `?` null-aware map-entry syntax already established elsewhere in this codebase, then 0 issues. `flutter build apk --debug` succeeded. `git status` confirms no Authentication/Wallet/Merchant/QR/Personal/Scheduled Payment file was touched — only new Analytics files plus the two route-mounting/rate-limiter additions and the deleted placeholder screen.
...
Phase 8.1 – Analytics Dynamic Data + Signup OTP Verification + UI Polish. Requested re-reading all six documentation files and the complete Analytics module before making changes, with no changes to completed payment architecture and no database changes unless absolutely required. Part 1 (highest priority): Analytics was showing static/demo values — a newly registered user saw the exact same analytics as another account. Find the exact root cause (do not assume) — check for dummy JSON, hardcoded provider values, non-userId-scoped backend queries, fake chart values, and uncleared provider cache after logout/login. Every logged-in user must have fully independent analytics; a new account must show all zeros, never reused values; empty states must show beautiful honest placeholders instead of fake charts. Part 2: redesign the Analytics screens to feel like Google Pay/CRED/Jupiter/Fi Money/PhonePe — large header, animated statistic cards, glass/premium cards, gradient balance cards, shimmer loading, animated progress bars, animated charts, achievement cards — presentation only, no backend change, stay consistent with the existing black+red theme, not colorful. Part 3: change Registration from Enter Details → Create Account → PIN → Biometric to Enter Details → Send OTP (Twilio) → OTP Verification Screen → Account Created → Create PIN → Enable Biometric → Dashboard, reusing the existing OTP/Twilio/rate-limiting/audit-logging infrastructure rather than duplicating it; a `User` row must not be created until the OTP is verified; the OTP screen needs 6-digit boxes, auto focus, countdown, resend, loading/success/failure animations. Part 4: if needed, use temporary signup session storage, never a permanent `User` before verification. Part 5/6: verify Login/Logout/PIN/Biometric/Transfer/Merchant/QR/Personal Payment/Scheduled Payment/Analytics/Dashboard/Wallet all still work, via `flutter analyze`, backend startup, and live HTTP tests covering User A/User B/new account/old account/logout-login, confirming no analytics leakage, no shared cache, working Twilio OTP delivery, and unchanged PIN/biometric flows. Part 7: update all three documentation files with a detailed implementation report. Keep `flutter analyze` at 0 issues; stop after this phase.

Result:

Entered plan mode given the genuine root-cause investigation and schema/cross-cutting-flow implications, audited the entire Analytics stack first (every backend query in `analytics.repository.js`/`analytics.service.js`, every controller action, the Flutter provider and all three Analytics screens), found the true root cause, then wrote the plan and got it approved with zero edits, followed by an explicit `AskUserQuestion` approval for the new `PendingRegistration` schema before migrating.

**Part 1 root cause** (confirmed, not assumed): every backend Analytics query was already correctly scoped by `req.user.id` — there was no dummy data, no hardcoded values, and no unscoped global query anywhere in the backend. The bug was entirely on the Flutter side: `main.dart` wraps the whole app in exactly one long-lived `ProviderScope`, never recreated for the app's lifetime, so `analyticsProvider` (and identically, `walletProvider`/`scheduledPaymentProvider`) — all plain `NotifierProvider`s, not `.autoDispose` — kept their in-memory state alive across logout/login. Three Analytics screens guard their initial fetch with `if (dashboard == null) loadAll()`, a reasonable "don't refetch every revisit" optimization within one session, but nothing anywhere in the codebase (confirmed via a `grep "ref.invalidate"` returning zero matches across the whole Flutter tree) ever reset that cached state on logout. So User A's cached `dashboard` survived into User B's session, and the guard — seeing it non-null — skipped reloading, showing User B User A's numbers. Fixed with a `_resetPerAccountProviders()` helper added to `auth_provider.dart` (`ref.invalidate(walletProvider)`, `ref.invalidate(scheduledPaymentProvider)`, `ref.invalidate(analyticsProvider)`), called at the end of `logout()`, `register()`, and `login()` — invalidating a non-autoDispose provider marks it dirty so the next read reruns `build()` and returns a genuinely fresh initial state, letting the exact same "only load if null" guards now correctly detect "nothing loaded yet" for the new account. This is the one Authentication-module file touched for Part 1; no Analytics backend file needed any change at all, since the bug was never there. A dead-code bug was also caught and fixed while addressing the "gracefully handle zero data" requirement: `MonthlySpendingLineChart`/`WeeklyExpenseBarChart` checked `points.isEmpty`, which could never be true since the backend's series builders always pre-seed every month/day bucket with zero — replaced with an all-zero check so the requested "No spending data — make your first payment to view spending trends" copy actually renders for a genuinely empty account. `PurposeWalletAnalyticsScreen`'s empty state and `PurposeWalletPieChart`'s empty branch were updated to the same real-data-only wording ("No wallet analytics — create your first Purpose Wallet").

**Part 2** (presentation only, same `analyticsProvider`/backend/models): `AnalyticsDashboardScreen` was fully rewritten with a gradient `_HeroBalanceCard` ("Financial Overview" + an animated `TweenAnimationBuilder` currency count-up + three dark stat tiles for Income/Expenses/Transfers), staggered `FadeSlideIn` stat cards, and an `_AchievementBadges` row where every badge ("On track with budgets," "No spending spikes," "All wallets active") is a plain boolean check against already-fetched real fields — never decorative or fabricated text, honoring "no dummy data anywhere in Analytics" even in the new UI. A pre-existing but previously unused `ShimmerBox` widget (`widgets/shimmer_box.dart`) was discovered and reused for loading skeletons across all four Analytics screens (extended additively with a new `ShimmerStatTile`), replacing plain spinners so the loading state doesn't jump when real content arrives. `PurposeWalletAnalyticsScreen`'s budget progress bar now animates via `TweenAnimationBuilder` (0 → real percentage, 800ms, easeOutCubic). The four `fl_chart` widgets each gained `duration`/`curve` constructor parameters for an animated draw-in — a one-line addition per chart, no data change. `InsightsScreen` and `MonthlyReportScreen` both gained matching shimmer skeletons and staggered entrance animations. Stayed strictly within the existing black+red `AppColors` palette throughout, per the explicit "do NOT make it colorful" instruction.

**Part 3/4 schema** (approved before migrating): a new `PendingRegistration` model (`fullName`/`email`/`phoneNumber`/`passwordHash`/`deviceId`/`deviceName`/`platform`/`fcmToken`/`otpAttempts` @default(0)/`status` (`OTP_SENT`/`VERIFIED`)/`expiresAt`/`createdAt`) mirroring `TransactionAuthSession`'s status/attempts/expiry shape, but carrying the full pending payload directly instead of an FK, since no `User` row exists yet to attach a session to. `passwordHash` is bcrypt-hashed before the row is ever written — no plaintext password persisted, even temporarily. Migration `20260716065213_add_pending_registration` applied via the normal `migrate dev --create-only` → `migrate deploy` → `generate` sequence. No existing table/column was altered; `auth.service.js`'s existing `register()`/`login()` and the `/auth/register` route were left completely intact and reachable — the new flow is a separate, additive `signup.repository/service/controller/routes/validation.js` module mounted at `/api/auth/signup`, and the Flutter registration screen simply stops calling the old endpoint in favor of the new ones.

`signup.service.js`'s `requestOtp()` reuses the exact same email/phone-uniqueness check `register()` already performs, hashes the password, replaces any prior pending row for that email, creates the `PendingRegistration` (5-minute expiry, matching Transaction Auth's existing `OTP_TTL_MINUTES`), and calls `twilioService.sendOtp()` — completely unmodified, the same function Transaction Authentication already uses for wallet-transfer OTPs. `verifyOtp()` enforces `otpAttempts < 5` (same `MAX_OTP_ATTEMPTS` constant already established for Transaction Auth) before calling Twilio, increments on a wrong code, and only deletes the pending row outright once the attempt cap is exceeded (a genuine lockout requiring a fresh signup) — an expired code is rejected for that one verify call only, without deleting the row, so the user can still tap "Resend" from the same OTP screen rather than being stranded (this exact behavior was a bug caught and fixed mid-implementation: an earlier version deleted the row on any expiry check, which would have made "Resend" impossible after expiry, contradicting the task's explicit "If OTP expires: Return to OTP screen"). On a correct code, `verifyOtp()` re-checks email/phone uniqueness defensively (the request-time check doesn't hold a lock across the OTP-entry gap), creates the real `User`, eagerly provisions the Main Wallet via the existing, unmodified `walletService.getMainWallet()`, calls the existing `deviceService.upsertDevice()`/`tokenService.issueTokenPair()` verbatim, deletes the `PendingRegistration` row, and returns the exact same `{user, accessToken, refreshToken}` shape `register()` already returns — so nothing downstream (Create PIN → Biometric → Dashboard) needed to change. `resendOtp()` re-sends via Twilio, resets attempts, and extends `expiresAt`. New pre-auth rate limiters were added since the existing OTP limiters are keyed by an authenticated `req.user.id`, which doesn't exist yet at signup: `signupOtpRequestLimiter` (email-keyed, reusing the existing `emailKeyGenerator` pattern) and `signupOtpVerifyLimiter` (a new `pendingRegistrationId`-keyed generator) — this is the "too many attempts → lock temporarily" enforcement at the network layer, on top of the in-row attempt cap. `auth.validation.js` gained two additive exports (`passwordSchema`, `deviceSchema`) so `signup.validation.js` could reuse them rather than duplicating validation rules.

Flutter: `RegisterScreen`'s submit handler now calls a new `authProvider.notifier.requestSignupOtp(...)` instead of `register(...)` directly, then navigates to a new `SignupOtpScreen` (`/verify-signup-otp`, added to the router's public routes since the user isn't authenticated yet) carrying the `pendingRegistrationId` and masked phone number. The new screen mirrors `TransactionAuthenticationScreen`'s existing OTP visual structure — the same reused 6-box `CodeInputField`, a countdown timer, "Resend," and the same `TweenAnimationBuilder` scale-in success/failure animations — with its own state machine underneath: `OTP_MAX_ATTEMPTS`/session-not-found errors show a "Start Over" failure screen popping back to Register, while `OTP_EXPIRED`/`OTP_INCORRECT` stay on the entry screen with an inline error and a cleared field. On successful verification, `authProvider.notifier.verifySignupOtp()` does exactly what `register()` already does (save session, reset device setup, set `status: onboarding`), so the router's existing onboarding redirect to Create PIN fires completely unchanged.

Testing (Part 5/6): a live Neon script confirmed the full OTP-signup happy path (request → verify with a stubbed Twilio check → exactly one `User` + one `MainWallet` created, tokens issued, `PendingRegistration` row deleted), 5 wrong codes correctly incrementing `otpAttempts` before a 6th attempt is blocked with `OTP_MAX_ATTEMPTS` (matching `transaction_auth.service.js`'s existing precedent, confirmed by fixing an off-by-one in the test's own expectation, not a service bug), and a dedicated resend-after-expiry test confirming an expired code is rejected without deleting the row, and a subsequent resend + correct code still succeeds. A final combined regression script registered User A through the new OTP flow and User B through the old, untouched `register()` endpoint, gave User A real transaction activity, and confirmed `analyticsService.getDashboardAnalytics()`/`getWalletAnalytics()` returned genuinely independent, non-equal figures for A vs. B — directly proving no cross-account leakage, with an explicit assertion that the two accounts' `totalIncome` values differ. `flutter analyze` reported one `invalid_constant` error on the first pass (a `const ShimmerBox(...)` call using `AppRadius.mdRadius`, a getter, not a compile-time constant), fixed by removing the erroneous `const`, then 0 issues. A backend module-load check confirmed `app.js` loads cleanly with the new routes mounted. `git status --porcelain` confirmed the diff touches only `analytics.*` Flutter screens/widgets, the new `signup.*` backend module, `auth.validation.js`/`rateLimit.middleware.js`/`app.js` (all additive), `schema.prisma` plus its migration, and the expected Flutter auth/router files — zero changes to any Wallet, Merchant Payment, QR Payment, Personal Payment, Scheduled Payment, or Transaction PIN file. `flutter build apk --debug` succeeded, confirming the app compiles end-to-end with all changes present. The physical test device was not connected this session, so an interactive tap-through of the new Signup OTP screen and the redesigned Analytics Dashboard could not be independently observed — disclosed honestly, the same pattern as every prior phase's live-device step.
...
Phase 8.1.1 — Fix Signup OTP + Riverpod Provider Crash. Do not continue to Phase 9. Two regressions from Phase 8.1: Issue 1 — after filling registration details the app showed "Can't send verification code," even though the transfer OTP already worked correctly through Twilio; requested tracing the complete flow (Register Screen → Signup Provider → Signup Service → Backend Endpoint → Twilio Service → Response) to find the exact failure point without guessing, returning the real backend error in debug mode if Twilio throws, reusing the exact same Twilio service used by Transaction Authentication rather than duplicating OTP logic, and verifying send/resend/cooldown/expiry/verification all work. Issue 2 — a `LateInitializationError: _repository has already been initialized` crash; requested auditing every `late` field inside WalletNotifier, AnalyticsNotifier, ScheduledPaymentNotifier, MerchantNotifier, PersonalPaymentNotifier, AuthNotifier, and any other Riverpod Notifier, replacing any late field assigned inside build() or assigned multiple times with a safe initialization strategy across the whole project (not just WalletNotifier), and verifying the provider lifecycle across logout/login/register/invalidation/rebuild/hot reload. Requested regression testing of Register OTP/Login/Wallet/Merchant/Personal/QR/Analytics/Scheduled Payment, `flutter analyze` at 0 issues, running the backend, and a live test of Signup OTP against Twilio. Stop after these two bugs are fixed; do not start the next phase. A follow-up message then supplied the live-Twilio-debugging result and narrowed the ask specifically to error-handling: return the real Twilio error code/message in development, map common Twilio errors to friendly messages (21608 → "This Twilio Trial account can only send OTP to verified phone numbers."), never expose Twilio internals in production (only "Unable to send verification code."), keep transfer OTP and signup OTP on the exact same Twilio service, and keep `flutter analyze` clean. Stop after improving error handling.

Result:

Traced the complete flow file-by-file (Register Screen → `AuthNotifier.requestSignupOtp` → `AuthService.requestSignupOtp` → `POST /auth/signup/request-otp` → `signup.service.js`'s `requestOtp()` → `twilio.service.js`'s `sendOtp()`) and confirmed every hop matched the plan exactly, with no duplicated OTP logic anywhere — both Signup OTP and Transaction Authentication call the identical, unmodified `twilioService.sendOtp()`/`checkOtp()`. Rather than guessing, called the Twilio Verify API directly (bypassing this app entirely) with the phone numbers already sitting in the dev database, and got back Twilio error 21608 on every one of them: "The phone number is unverified. Trial accounts cannot send messages to unverified numbers." A follow-up check of the account's Outgoing Caller IDs via the Twilio API confirmed the account is a Trial account with exactly one verified number. This proved the "bug" was never a code defect — signup OTP was failing for the same structural reason it would fail for any brand-new phone number on a trial Twilio account; the previous behavior of swallowing every Twilio error into one generic message just made this indistinguishable from an actual defect.

Per the follow-up's explicit scope (do not modify the signup flow, OTP flow, or Twilio integration itself — only the error handling), the fix touched exactly two files: `AppError` (`backend/src/utils/appError.js`) gained an optional 4th `details` constructor param (already supported by `error.middleware.js`, which forwards `err.details` when present — it just had never been populated), and `twilio.service.js`'s `sendOtp()` catch block now branches on `NODE_ENV`: in production it throws the same generic `"Unable to send verification code."` with no details, exactly as before; outside production it maps common Twilio codes to friendly wording (21608 the trial-account case above, plus 21211 invalid-number and 60203 rate-limited) and always attaches the raw `{twilioCode, twilioMessage}` as `details` for debugging, falling back to Twilio's own message text (never a placeholder) for any code without a specific mapping. `checkOtp()`, `signup.service.js`, and `transaction_auth.service.js` were not touched at all — confirmed via `git diff --stat`, so signup OTP and transfer OTP continue to share the exact same Twilio service unmodified. Live-verified via curl against a running backend: an unverified number now returns the friendly 21608 message plus real Twilio code/message in `details`; forcing `NODE_ENV=production` in an isolated process returns only the generic message with no `details` key at all.

For Issue 2, read Riverpod 3.3.2's own source (`flutter_riverpod`/`riverpod` v3.3.2, resolved via `.dart_tool/package_config.json`) to find the real mechanism rather than guessing: `$ClassProviderElement.create()` memoizes the Notifier instance on `classListenable.result` and only constructs a new one if that field is still null; `invalidateSelf()` (what `ref.invalidate()` calls) only tears down and recreates the element if it has no remaining listeners at the moment of invalidation (`mayNeedDispose()`) — if the provider still has an active listener (e.g. a mounted screen, or another provider watching it), the existing element and its existing Notifier instance survive, and the next `flush()` simply re-invokes `build()` on that same instance. Every Notifier in this codebase (`Wallet`, `Analytics`, `ScheduledPayment`, `Merchant`, `PersonalPayment`, `Auth`, `Qr`, `TransactionAuth`) had a `late final X _repository` (or, for `AuthNotifier`, three such fields) assigned inside `build()` — safe only under the assumption that `build()` never reruns on the same instance, which the framework source shows is false whenever a listener is still attached. This is exactly what `AuthNotifier._resetPerAccountProviders()` (added in Phase 8.1) does on every logout/login/register: invalidates `walletProvider`/`scheduledPaymentProvider`/`analyticsProvider` while a screen further down in the widget tree may well still be watching one of them, reproducing the crash.

Fixed all 8 Notifiers identically: every `late final X _repository` field (and `AuthNotifier`'s three fields) became a plain getter (`X get _repository => ref.read(xProvider)`) with no backing field at all, so there is nothing to "already be initialized" no matter how many times `build()` runs on the same instance. `WalletNotifier`, `AnalyticsNotifier`, and `ScheduledPaymentNotifier` (the three actually invalidated today) were the direct fix for the reported crash; `MerchantNotifier`, `PersonalPaymentNotifier`, `QrNotifier`, `TransactionAuthNotifier`, and `AuthNotifier` itself were fixed the same way defensively, per the explicit "audit the whole project, don't just patch WalletNotifier" instruction, since any of them would carry the identical latent bug the moment they're invalidated in the future. A follow-up grep confirmed zero remaining `late` fields inside any provider file.

Testing: `flutter analyze` → 0 issues both before and after the Notifier changes. Backend module-load check passed. Debug/dev-mode and forced-production-mode error responses were both verified live via curl/isolated Node process as described above, then temporary debug `PendingRegistration` rows were deleted and the throwaway test backend process was stopped, leaving no test artifacts behind.
...
Phase 8.1.2 — Remove Signup OTP (Keep All Other Security Intact). Do not start the next phase. Explicitly framed as a development-only change to make testing easier: remove the Signup OTP verification flow only, restoring Register → Create Account → Create App PIN → Enable Biometric → Login → App PIN/Biometric (i.e. undoing Phase 8.1's Enter Details → Send OTP → Verify OTP → Create Account → PIN → Biometric flow). Requested removing the `PendingRegistration` flow, the Signup OTP screen, its provider methods, its routes/API calls, resend, cooldown timer, and verification logic, while keeping Login, App PIN, Biometric unlock, Payment PIN, Main Wallet security, Wallet Transfer, Merchant Payment, QR Payment, User→User Payment, Scheduled Payment, Transaction Authentication, and the Twilio OTP service (for Wallet Transfer and any future use) completely untouched. Registration should call the existing account-creation endpoint directly again (User + Main Wallet + default records), then continue to Create App PIN → Enable Biometric exactly like before. Backend cleanup should remove only unused Signup OTP code, with no schema changes unless `PendingRegistration` is now completely unused — if so, remove it and generate a migration. Flutter cleanup should remove the Signup OTP screens/provider/service methods/routes and restore Register to submit directly. Requested verifying Register, Create App PIN, Enable Biometric, Login, Logout, Re-login, App restart, Wallet, Merchant Payment, QR Payment, Personal Payment, Scheduled Payment, and Analytics all continue working; running `flutter analyze` to 0 issues; running the backend; live-testing registration; and confirming Register → Create PIN → Enable Biometric → Dashboard works exactly like before. Update README.md/TASKS.md/PROMPT_HISTORY.md and stop after this cleanup.

Result:

Confirmed `PendingRegistration` was used exclusively by the signup module being removed (no other model/service referenced it) and that `AuditLog.action` is a plain `String` column, not an enum, so historical `SIGNUP_OTP_*` audit rows from testing remain valid, inert history with no migration risk. Backend removal: deleted `signup.service.js`, `signup.repository.js`, `signup.controller.js`, `signup.routes.js`, and `signup.validation.js` outright; removed the `signupRoutes` import and its `/api/auth/signup` mount from `app.js`; removed `signupOtpRequestLimiter`/`signupOtpVerifyLimiter`/`pendingRegistrationKeyGenerator` from `rateLimit.middleware.js`; reverted the `passwordSchema`/`deviceSchema` additive exports from `auth.validation.js` since `signup.validation.js` (their only external consumer) no longer exists — the schemas themselves stay, only their now-dead re-export was removed. `auth.service.js`, `auth.controller.js`, and `auth.routes.js` were not touched at all (confirmed by `git show HEAD:backend/src/utils/auth.validation.js` matching the post-revert file byte-for-byte, since Phase 8.1's addition and this session's removal cancelled out relative to the last commit).

Schema: removed the `PendingRegistration` model and `PendingRegistrationStatus` enum from `schema.prisma`. `prisma migrate dev --create-only` hung waiting on an interactive confirmation (dropping a table), so — following the same established workaround as prior sessions' awkward migrations — hand-wrote `prisma/migrations/20260716140000_remove_pending_registration/migration.sql` (`DROP TABLE "PendingRegistration"; DROP TYPE "PendingRegistrationStatus";`) directly; the backgrounded `migrate dev` process picked it up mid-run, applied it, and then generated its own now-redundant empty migration folder (since by the time it diffed, the drop had already been applied) — that empty duplicate was deleted, `prisma migrate status` confirmed "Database schema is up to date!", and `prisma generate` refreshed the client. Three genuinely orphaned `PendingRegistration` rows from earlier testing were dropped along with the table — expected, since none of them had ever produced a real `User`.

Flutter removal: deleted `signup_otp_screen.dart` and `pending_registration_model.dart`; removed `requestSignupOtp`/`verifySignupOtp`/`resendSignupOtp` from both `AuthNotifier` (`auth_provider.dart`) and `AuthService` (`auth_service.dart`), along with the now-unused `PendingRegistrationModel` import in each; removed the three `signupRequestOtp`/`signupVerifyOtp`/`signupResendOtp` constants from `api_constants.dart`; removed the `/verify-signup-otp` route, its `SignupOtpScreen` import, and its `_publicRoutes` entry from `app_router.dart`. `RegisterScreen._submit()` was reverted to call `ref.read(authProvider.notifier).register(...)` directly instead of `requestSignupOtp(...)`, with no manual navigation afterward — the router's existing `AuthStatus.onboarding` redirect (unchanged throughout every phase) already sends any onboarding-status location straight to `/create-pin`, so this needed no router logic change beyond deleting the OTP route. The Analytics-cache-leak fix (`_resetPerAccountProviders()`) and the Notifier `late`-field-to-getter fix (both from Phase 8.1/8.1.1) were left completely in place, since neither depends on the signup-OTP flow and both are still required. A final grep for `signup`/`Signup`/`PendingRegistration`/`pendingRegistration` across both `mobile/lib` and `backend/src` returned zero matches.

Testing: `flutter analyze` → 0 issues. Backend module-load check passed. Live-tested against a running backend: `POST /auth/signup/request-otp` now correctly 404s (route gone); `POST /auth/register` with a fresh account succeeds exactly as before, returning `{user, accessToken, refreshToken}`; confirmed the Main Wallet is (correctly) *not* created eagerly by this plain `register()` path — it never was, prior to Phase 8.1's since-reverted eager-provisioning addition — and instead gets lazily created on first use, verified by immediately calling `GET /wallet/dashboard` with the fresh account's token and seeing a real, zero-balance Main Wallet auto-provisioned. Smoke-tested Login, Merchant List, Analytics Dashboard, and Scheduled Payment Dashboard against a fresh account, all returning `success: true` with no errors — none of those modules' files were touched by this session's diff (confirmed via `git status --porcelain`), so this served as confirmation rather than a fix. All throwaway test accounts created during this session's live testing were deleted afterward (in FK-safe order: RefreshToken → Device → AuditLog → MainWallet → User) and the temporary test backend process was stopped, leaving no test artifacts behind.
